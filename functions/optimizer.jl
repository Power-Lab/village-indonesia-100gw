function capacity_expansion(inputs, mipgap, CO2_constraint, CO2_limit, RE_constraint, RE_limit, Grid, VillageBuild, ImportPrice, NoCoal, CO235reduction, BAUCO2emissions; village_storage_max_mwh = 208.0)
    ...
    @constraints(CE, begin
            ...
            #define the commit variable, normal
            cCommitState[t in inputs.INTERIOR, g in inputs.UC],
                vCOMMIT[t,g] == vCOMMIT[t-1,g] + vSTART[t,g] - vSHUT[t,g]

            #define the commit variable, sub-period wrapping (rep weeks are
            #independent samples, so the commitment recursion must be cyclic
            #within each period rather than threaded across the whole horizon)
