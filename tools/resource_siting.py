...
def _load(gis_dir, metric_crs):
    import geopandas as gpd
    cand = gpd.read_file(os.path.join(gis_dir, "candidate_solar_vector.gpkg"))
    if "solar_MW" not in cand.columns:
        sys.exit("candidate_solar_vector.gpkg has no 'solar_MW' column")
...
def _points_from_csv(path, metric_crs):
    ...
    df = pd.read_csv(path)
    lat = next((c for c in ("lat", "Latitude", "latitude", "LAT") if c in df.columns), None)
    lon = next((c for c in ("lon", "Longitude", "longitude", "LON", "lng") if c in df.columns), None)
...
def cmd_run(args):
    import pandas as pd
    cand, subs = _load(args.gis_dir, args.metric_crs)
